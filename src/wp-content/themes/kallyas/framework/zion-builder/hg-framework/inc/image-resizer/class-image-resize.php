...
function mr_image_resize($url, $width=null, $height=null, $crop=true, $align='c', $retina=false) {
  ...
  // ... Otherwise, return error, null or image
	...
	$force_image_size = apply_filters('hg_force_image_resize', false);

  if( $force_image_size && ( $orig_width < $width ) ) {
...
function mr_common_info($url, $width, $height, $crop, $align, $retina) {
  ...
  if (preg_match('/\/[0-9]{4}\/[0-9]{2}\/.+$/', $urlinfo['path'], $matches)) {
	...
	// Get uploads dir config
	$wp_uploads_url = $wp_upload_dir['baseurl'];
	$wp_upload_path = $wp_upload_dir['basedir'];

	// Get file path relative to uploads dir
	$url_path = str_replace( $wp_upload_dir['baseurl'], '', $url );
	$file_path = $wp_upload_path . $url_path;

	// If the file doesn't exists, use the old wayto get the image
...
function zn_get_post_image( $post_id , $width, $height=0, $attr = array() , $popup = false ){
...
function zn_get_image( $attachment_id , $width, $height=0, $attr = array() , $popup = false ) {
