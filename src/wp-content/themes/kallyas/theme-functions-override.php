...
if ( ! function_exists( 'get_video_from_link' ) ) {
	...
	function get_video_from_link( $string, $css = null, $width = 425, $height = 239, $video_attributes = null ) {
		...
		if ( ! empty( $video_attributes ) && is_array( $video_attributes ) ) {
			$extra_options[] = 'autoplay=' . ( ! empty( $video_attributes['autoplay'] ) ? $video_attributes['autoplay'] : 0 );
			$extra_options[] = 'loop=' . ( ! empty( $video_attributes['loop'] ) ? $video_attributes['loop'] : 0 );
			$extra_options[] = 'controls=' . ( ! empty( $video_attributes['controls'] ) ? $video_attributes['controls'] : 0 );
			$extra_options[] = 'mute=' . ( ! empty( $video_attributes['mute'] ) ? $video_attributes['mute'] : 0 );
		}
		...
		if ( 'www.youtube.com' == $video_url['host'] || 'youtube.com' == $video_url['host'] || 'www.youtu.be' == $video_url['host'] || 'youtu.be' == $video_url['host'] ) {
			...
			$string = '<iframe class="' . $css . '" width="' . $width . '" height="' . $height . '" src="//www.youtube.com/embed/' . $matches[0] . '?iv_load_policy=3&amp;enablejsapi=0&amp;wmode=opaque&amp;feature=player_embedded&amp;' . implode( '&amp;', $extra_options ) . '" allow="autoplay; fullscreen"></iframe>';
			...
			$string = preg_replace(
				...
				'<iframe ' . WpkPageHelper::zn_schema_markup( 'video' ) . ' class="youtube-player ' . $css . '" src="//player.vimeo.com/video/$2?' . implode( '&amp;', $extra_options ) . '" width="' . $width . '" height="' . $height . '" allow="autoplay; fullscreen"></iframe>', $string );
		} else {
...
if ( ! function_exists( 'wpkRegisterSidebars' ) ) {
	...
	function wpkRegisterSidebars() {
		if ( function_exists( 'register_sidebar' ) ) {
			...
			$f_row1 = key( json_decode( stripslashes( $footer_row1_widget_positions ), true ) );
			if ( $f_row1 > 1 ) {
			...
			$json_row1_config = json_decode( stripslashes( $footer_row2_widget_positions ), true);

			if ($json_row1_config) {
				$f_row1 = key( $json_row1_config );
				if ( $f_row1 > 1 ) {
					register_sidebars( $f_row1, array(
